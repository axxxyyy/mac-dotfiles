...
in
{
  ...
  xdg.configFile."sketchybar/sketchybarrc" = {
    ...
    text = ''
      #!/bin/bash
      export CONFIG_DIR="${config-dir}/share/sketchybar"

      source "$CONFIG_DIR/appearance/colors" # Loads all defined colors
      source "$CONFIG_DIR/appearance/icons" # Loads all defined icons

      ITEM_DIR="$CONFIG_DIR/items" # Directory where the items are configured
      PLUGIN_DIR="$CONFIG_DIR/plugins" # Directory where all the plugin scripts are stored

      FONT="Iosevka" # Needs to have Regular, Bold, Semibold, Heavy and Black variants
